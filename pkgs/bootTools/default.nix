...
let
    ...
    setup = builtins.fromJSON (builtins.readFile ./static/setup.json);
in
    {
        "darwin-aarch-64" = (
            let
                system = "aarch64-darwin";
                thisSetup = setup.osSpecific.${system};
                bootstrapFiles = {
                    bootstrapTools = builtins.fetchurl thisSetup.stdFetchurls.bootstrapTools;
                    unpack         = builtins.fetchurl thisSetup.stdFetchurls.unpack;
                };
            in
                builtins.derivation {
                    system = system;
